...
class ItemSearchTest extends TestCase
{
    ...
    public function test_can_search_items_by_name()
    {
        ...
        $targetItem = Item::factory()->create(['name' => '黒色のキーボード']);
        $otherItem = Item::factory()->create(['name' => '白色のマウス']);

        // 2. 実行：検索クエリを投げる（inputのname属性が 'query' の場合）
    ...
    public function test_search_keyword_is_kept_in_mylist_tab_link()
    {
        ...
        // あなたのBladeコードでは 'keyword' を使用しているので、パラメータ名を合わせます
        $response = $this->get('/?keyword=' . urlencode($keyword));

        $response->assertStatus(200);

        // 3. 検証（プロの書き方）
        // 期待されるフルURLを生成し、HTMLエスケープ（& -> &amp;）をシミュレートする
        $expectedUrl = str_replace('&', '&amp;', route('item.index', [
            'tab' => 'mylist',
            'keyword' => $keyword
        ]));

        // href属性として存在するか、エスケープを考慮してチェック（falseで生HTMLを比較）
